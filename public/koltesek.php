...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["add_submit"])) {
    $osszeg = $_POST["osszeg"];
    $kategoria_nev = trim($_POST["kategoria_nev"] ?? "");
    $megjegyzes = $_POST["megjegyzes"];
    $datum = $_POST["datum"];

    if ($kategoria_nev === "") {
        $hiba = "Kérjük, válassz kategóriát.";
    } else {
        try {
            $katStmt = $pdo->prepare("SELECT id FROM kategoriak WHERE felhasznalo_id = ? AND nev = ?");
            $katStmt->execute([$user_id, $kategoria_nev]);
            ...
            if ($kat) {
                ...
                $insertKat->execute([$kategoria_nev, $user_id]);
...
$stmt_kat = $pdo->query("SELECT DISTINCT kat_nev FROM termekek WHERE kat_nev IS NOT NULL AND kat_nev != '' ORDER BY kat_nev");
$kategoriak = $stmt_kat->fetchAll(PDO::FETCH_COLUMN);
...
<nav>
    ...
    <form method="POST">
        ...
        <select name="kategoria_nev" required>
